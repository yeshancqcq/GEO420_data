...
# Thus:
# A linear normalization curve y = kx + b passes through (30000, 1) and (0, 0.1)
# So the equation of this curve is: y = 0.00003x + 0.1 where y is the possibility for keeping a record and x is the years bp

total_record <- nrow(svz)
for(i in 1:total_record){
  svz$Linear_Prob[i] <- 0.00003 * svz$YearsBP[i] + 0.1
}
