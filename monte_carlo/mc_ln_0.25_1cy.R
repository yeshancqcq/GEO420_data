...
# y = ln(x+b)/k + c passes through (30000, 0.25) and (0, 1)
# The possibility for a record to be KEPT at each age is:
# y =  ln( x + 1 ) / 13.7345 + 0.1

total_record <- nrow(svz)
for(i in 1:total_record){
  svz$Linear_Prob[i] <- log(svz$YearsBP[i] + 1) / 13.7345 + 0.25
}
...
plot <- plot + scale_x_reverse(limits = c(30000, 0.1),breaks = scales::pretty_breaks(n = 20)) +
  ggtitle("Number of SVZ Events Over Time, Normalized to 30000 Years BP (Natural Log)","Monte Carlo Simulation for 1000 Times, Assume 25% Records Are Preserved at 30000 Years BP") +
