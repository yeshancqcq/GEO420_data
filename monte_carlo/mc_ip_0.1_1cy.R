...
# y = k/(x-b) passes through (30000, 0.1) and (0, 1)
# So the equation of this curve is: y = 3333.33/(x+3333.33) where y is the possibility for a record that is kepted in real world and x is the years bp
# Then: the possibility of each record that should be DROPPED in our analysis (let y = 0 when x = 30000):
# y = 3333.33/(x+3333.33) - 0.1
# Therefore: the possibility for a record to be KEPT at each age is:
# y = 1 - (3333.33/(x+3333.33) - 0.1)

total_record <- nrow(svz)
for(i in 1:total_record){
  svz$Linear_Prob[i] <- 1 - (3333.33/(svz$YearsBP[i] + 3333.33) - 0.1)
}
...
plot <- plot + scale_x_reverse(limits = c(30000, 0.1),breaks = scales::pretty_breaks(n = 20)) +
  ggtitle("Number of SVZ Events Over Time, Normalized to 30000 Years BP (Inverse Proportioanl)","Monte Carlo Simulation for 1000 Times, Assume 10% Records Are Preserved at 30000 Years BP") +
