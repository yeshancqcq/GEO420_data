...
# Calculate the probability of keeping the record for each record in the dataset (Ln function)
...
# 2) We assign a 10% possibility that the record at 0 years bp would be kept, and
# 3) For every record in between, the possibility of being-kept is calculated from a Ln curve 
# Thus:
# A Ln normalization curve for a record that is preserved in our assuption at each time:
# y = ln(x+b)/k + c passes through (30000, 0.1) and (0, 1)
# The possibility for a record to be KEPT at each age is:
# y =  ln( x + 1 ) / 11.45433 + 0.1

total_record <- nrow(svz)
for(i in 1:total_record){
  svz$Linear_Prob[i] <- log(svz$YearsBP[i] + 1) / 11.45433 + 0.1
}
...
plot <- plot + scale_x_reverse(limits = c(30000, 0.1),breaks = scales::pretty_breaks(n = 20)) +
  ggtitle("Number of SVZ Events Over Time, Normalized to 30000 Years BP (Natural Log)","Monte Carlo Simulation for 1000 Times, Assume 10% Records Are Preserved at 30000 Years BP") +
